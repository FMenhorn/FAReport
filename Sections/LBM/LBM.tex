...
of this method can be attributed to its simplicity.The code we implemented was based on a D2Q9 stencil, i.e a 2D grid with 9 partial differential formulas(PDFs)\todointern{Mohamed}{PDF is partial differential "formula". Is that a common term ? I am more used to hear partial differential "equations" PDEs.} per cell (See Figure \ref{fig: D2Q9 push scheme}). 
...
\subsection *{Stream}
 ...
 The collision is a local operation to each cell and it just requires plugging in the formula(1) as described in the Algorithm section\todointern{Mohamed}{Isn't it better to write this as a reference? rephrasal suggestion: The collision is a local operation which requires solving equation \ref{eq:LBMalgorithm1} presented in LBM Algorithm section [\ref{sec:LBM_algorithm}]}.The computation of macroscopic density and velocity needed to calculate equilibrium are carried out using:
...
\end{tabular}
\todointern{Mohamed}{ the figure in the first point is figure \ref{fig: Moving boundary} ?}
